...
exports.upload = function (options) {
  ...
  if (options.queueSize || options.concurrency) {
    uploadOptions.queueSize = options.queueSize || options.concurrency || 4;
  }

  if (options.partSize) {
    uploadOptions.partSize = options.partSize || 5242880;
  }
  ...
  //
  // Set these options so they are now configurable.
  // In the future these will be passed to the s3.upload method
  //
  writableStream.concurrentParts(uploadOptions.queueSize);
  writableStream.maxPartSize(uploadOptions.partSize);
